...
import { annulerMission } from "@/app/actions/missions";

export function AnnulerMissionButton({ missionId }: { missionId: string }) {
  ...
  async function handleClick() {
    setEnvoi(true);
    setErreur(null);
    const result = await annulerMission(missionId);
    setEnvoi(false);
    if (result.success) {
      router.refresh();
    } else {
